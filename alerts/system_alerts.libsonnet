        ...
        rules: [
          {
            ...
            expr: |||
              count(count by (%(clusterLabel)s, gitVersion) (label_replace(kubernetes_build_info{%(notKubeDnsCoreDnsSelector)s},"gitVersion","$1","gitVersion","(v[0-9]*.[0-9]*).*"))) by (%(clusterLabel)s) > 1
            ||| % $._config,
            'for': '15m',
            labels: {
              severity: 'warning',
